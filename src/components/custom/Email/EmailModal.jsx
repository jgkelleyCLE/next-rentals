...
const EmailModal = ({ subtotal, deliveryFee, taxPrice, total }) => {
  ...
  const [createOrder, { data: orderData, isLoading: orderLoading, isSuccess: orderSuccess, isError: orderError, error: orderErrorData }] = useCreateOrderMutation()
  ...
  <p className="bg-blue-500 hover:bg-blue-600 transition duration-300 p-2 rounded-md mt-2 mb-1 w-full text-white">Email Quote</p>
  </DialogTrigger>
  <DialogContent className="w-11/12 max-w-[800px]">
    ...
    <form className="flex flex-col items-center w-full" onSubmit={submitHandler}>
            <input required className="border-2 border-gray-300 p-2 rounded-md my-1 w-full" placeholder='Event Title' type="text" value={title} onChange={ (e)=> setTitle(e.target.value) } />
                ...
                <DayPicker
                    ...
                    footer={footer}
                    />
                <input required className="border-2 border-gray-300 p-2 rounded-md my-1 w-full" placeholder='Your email address' type="text" value={email} onChange={ (e)=> setEmail(e.target.value) } />
                <button className="bg-safariOrange hover:bg-safariOrangeHover text-white w-full p-2 rounded-md" type="submit">Submit</button>
            </form>
