    ...
    <div>
      ...
      <FlexColumn className="">
            ...
            <CartCard key={item._id} className="relative " >
              ...
              <div className="flex items-start">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <h1 className="text-lg font-semibold text-gray-600 dark:text-gray-400">${item.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h1>
                    <QuantityContainer>
                    <button onClick={()=> dispatch(decreaseQuantity(item))}><MinusIcon /></button>
                    <h1 className="w-[50px] border-2 border-gray-400 px-2 rounded-md">{item.cartQuantity}</h1>
                    
                    <button onClick={()=> dispatch(increaseQuantity(item))}><PlusIcon /></button>
                  </QuantityContainer>
                  </div>
              </div>
        ...
        <ClearCartModal />
          ...
          <h1 className="font-bold">Total: ${total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} </h1>
