...
const CartDrawer = () => {
  ...
  const total = subtotal + deliveryFee + taxPrice
    ...
    const list = sortedCart?.map(item => (
        <div key={item.id} className="bg-gray-200 dark:bg-gray-800 dark:text-gray-200 p-2 rounded-md flex items-center justify-start my-2 relative cursor-pointer hover:bg-gray-100 transition duration-300" onClick={()=> navigationHandler(item)}>
            ...
            <div className="flex flex-col">
                ...
                <h1 className="text-sm">${item.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h1>
  ...
  return (
    <DrawerRoot size="md" open={open} onOpenChange={(e) => setOpen(e.open)}>
      ...
      <DrawerContent>
        ...
        <DrawerBody>
        {cart.length === 0 ? <FlexColumn><h1 className="text-xl text-gray-400 italic">Cart is empty</h1><OrangeButton onClick={()=> productHandler()}>View Products</OrangeButton></FlexColumn> : null}
        {list}
          
        </DrawerBody>
        <DrawerFooter>
          <div className="flex flex-col items-end gap-2">
            <h1 className="text-2xl">Subtotal: ${subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h1>
          ...
          <OrangeButton className="bg-gray-400 hover:bg-gray-300 transition duration-300 mt-0" mr={3} onClick={()=> setOpen(false)}>
              Cancel
            </OrangeButton>
            <OrangeButton className="mt-0" onClick={cartHandler}>View Quote</OrangeButton>
          </div>
          </div>
