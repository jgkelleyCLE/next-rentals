...
const AllOrders = () => {
  ...
  if (isSuccess) {
    content = filteredOrders?.map((item) => (
      <Table.Row key={item._id}>
        ...
        <Table.Cell className="hidden md:table-cell" textAlign="end">
          {' '}
          ${item.totalPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </Table.Cell>
        <Table.Cell textAlign="end">
          <Link className="text-blue-500" href={`/order-details/${item._id}`}>
            Edit
          </Link>
        </Table.Cell>
      </Table.Row>
  ...
  return (
    <div className="w-full flex flex-col mx-auto">
      All Orders
      <input
        placeholder="Filter orders by status, title, or contain a certain product..."
        value={query}
